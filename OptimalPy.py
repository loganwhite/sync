...
class OptimalBoundNear(Optimal):
    ...
    def math_solution(self):
        ...
        #         name="default_controller_mapping_relationship")
            ...
            model.addConstrs(x[i] == 1 for i in self.failed_switches)
            ...
            # print("solving math ends")
            self.opt_end_time = time.time()
            # print("solution time ddddddd: %s" % (self.opt_end_time - self.opt_start_time))
            status = model.status
            print("name: %s, status = %s" % (self.name, status))
            if status == 3:
                return
...
class LP(Optimal):
    ...
    def get_variables(self,model):
        # x = model.addVars(self.location_num, lb=0, ub=1, vtype=GRB.CONTINUOUS, name="x")
        d = model.addVars(self.flow_num, lb=0, ub=1, vtype=GRB.CONTINUOUS, name="d")
        z = model.addVars(self.location_num, self.location_num, 
            lb=0, ub=1, vtype=GRB.CONTINUOUS, name="z")
    ...
    def math_solution(self):
        ...
        # Create a new model
            ...
            model.setParam( 'OutputFlag', False ) 
        # Create variables
            z, d = self.get_variables(model)
        
        # Add constraints
            model.addConstrs((quicksum(z[i, j]
                for j in self.available_controllers) >= 0
                for i in self.failed_switches),name="mapping0")

            model.addConstrs((quicksum(z[i, j]
                for j in self.available_controllers) <= 1
                for i in self.failed_switches),name="mapping1")
